...
export default function PublicTrackingPage() {
  ...
  const fetchTracking = () => {
    ...
    api.get(`/tracking/public/${token}`)
      .then((res) => {
        ...
        .catch(() => setError(t('publicTracking.invalidLink')));
  };
    ...
    return (
      <div className={styles.errorContainer}>
        <div className={styles.errorCard}>
          <h2>{t('publicTracking.unavailable')}</h2>
          <p>{error}</p>
        </div>
      </div>
    );
  ...
  if (!delivery) {
    return <div className={styles.loadingContainer}>{t('publicTracking.loading')}</div>;
  }
    ...
    <div className={styles.page}>
      <div className={styles.header}>
        <div className={styles.headerRow}>
          ...
          <span className={styles.liveLabel}>{t('publicTracking.live')}</span>
          {lastUpdate && <span className={styles.lastUpdate}>{t('publicTracking.updated', { time: lastUpdate })}</span>}
        </div>
        <p>{t('publicTracking.status')} : <strong>{t(`deliveries.status.${delivery.status}`, { defaultValue: delivery.status })}</strong></p>
        <p>{t('publicTracking.pickup')} : {delivery.pickupAddress}</p>
        <p>{t('publicTracking.delivery')} : {delivery.deliveryAddress}</p>
        {currentPos?.speed !== undefined && currentPos?.speed !== null && <p>{t('publicTracking.currentSpeed')} : {(currentPos.speed * 3.6).toFixed(1)} km/h</p>}
      </div>
        ...
        <MapContainer center={center} zoom={13} maxZoom={Math.max(...Object.values(TILE_PROVIDERS).map((p) => p.maxZoom))} style={{ height: '100%', width: '100%' }}>
          ...
          {currentPos && (
            <Marker position={[currentPos.latitude, currentPos.longitude]}>
              <Popup>
                {t('publicTracking.currentPosition')}<br />
                {t('publicTracking.speed')} : {((currentPos.speed ?? 0) * 3.6).toFixed(1)} km/h<br />
          ...
          {delivery.pickupLat && delivery.pickupLng && (
            <Marker position={[delivery.pickupLat, delivery.pickupLng]} icon={deliveryIcon}>
              <Popup>{t('publicTracking.pickup')} : {delivery.pickupAddress}</Popup>
            </Marker>
          )}
          {delivery.deliveryLat && delivery.deliveryLng && (
            <Marker position={[delivery.deliveryLat, delivery.deliveryLng]} icon={deliveryIcon}>
              <Popup>{t('publicTracking.delivery')} : {delivery.deliveryAddress}</Popup>
            </Marker>
          )}
        </MapContainer>
      </div>
