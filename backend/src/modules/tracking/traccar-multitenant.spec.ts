...
describe('Traccar multi-tenant isolation', () => {
  ...
  it('rejects linking the same traccarDeviceId to two vehicles', async () => {
    ...
    await expect(service.linkVehicleToTraccar(VEHICLE_B, COMPANY_B, TRACCAR_ID)).rejects.toThrow(
      ConflictException,
    );

    const e = await service.linkVehicleToTraccar(VEHICLE_B, COMPANY_B, TRACCAR_ID).catch((e) => e);
  ...
  it('allows different traccarDeviceIds for different vehicles', async () => {
    ...
    await expect(service.linkVehicleToTraccar(VEHICLE_A, COMPANY_A, 'device-1')).resolves.toEqual({
      id: 'updated',
    });

    await expect(service.linkVehicleToTraccar(VEHICLE_B, COMPANY_B, 'device-2')).resolves.toEqual({
      id: 'updated',
    });
  });
});
