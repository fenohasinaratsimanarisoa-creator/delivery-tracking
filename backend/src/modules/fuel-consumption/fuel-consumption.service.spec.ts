...
describe('FuelConsumptionService', () => {
  ...
  describe('gpsDataQuality suspicious — dérive GPS stationnaire détectée (audit 2026-08-27)', () => {
    ...
    it('marque suspicious : nombreux allers-retours (~300m), accuracy dégradée (40m), déplacement net minuscule', async () => {
      ...
      // AUDIT 2026-08-28 (révisé) : le coût EST calculé et affiché même quand
      // gpsDataQuality='suspicious' — la distance est bien meilleure depuis le
      // correctif de bruit GPS, et un montant indicatif + badge ⚠️ vaut mieux
      // qu'un « — ». Seul un prix carburant ABSENT (électrique) donne 0/null.
      expect(captured.create.estimatedCost).toBeGreaterThan(0);
      expect(captured.create.pricePerLiterUsed).toBe(5000);
    });
  ...
  describe('getPeriodSummary — synthèse carburant par période', () => {
    ...
    it('source=gps : agrège les DailyFuelReport (distance GPS, coût estimé, litres dérivés)', async () => {
      ...
      expect(res.mixedSettings).toBe(false); // même conso (5) sur les 2 rapports
      const sep = res.buckets.find((b) => b.key === '2026-09')!;
    ...
    it('source=gps : mixedSettings=true quand la conso théorique change en cours de période', async () => {
      mockPrisma.dailyFuelReport.findMany.mockResolvedValueOnce([
        {
          reportDate: new Date('2026-09-05T00:00:00.000Z'),
          distanceKm: 30,
          consumptionLPer100Km: 2,
          estimatedCost: 2940,
          pricePerLiterUsed: 4900,
          gpsDataQuality: 'sufficient',
        },
        {
          reportDate: new Date('2026-09-10T00:00:00.000Z'),
          distanceKm: 30,
          consumptionLPer100Km: 3.5,
          estimatedCost: 5565,
          pricePerLiterUsed: 5300,
          gpsDataQuality: 'sufficient',
        },
      ]);
      ...
      expect(res.mixedSettings).toBe(true);
    });
